...
class TMSG():
  ...
  def msgPrintStarted(self, payload, **kwargs):
    self._logger.debug('Print start. Auto turn on: ' + str(self.main._settings.get_boolean(['auto_turn_on'])))
    self.last_z = 0.0
    self.last_notification_time = time.time()
    self._sendNotification(payload, **kwargs)
    if self.main._settings.get_boolean(['auto_turn_on']):
      if not self.main.tcmd:
        self._logger.debug("Cannot auto-turn on")
        return
      self.main.tcmd.commandDict['/on']['cmd'](None,None,None, None)
  ...
  def msgPrintDone(self, payload, **kwargs):
    ...
    self._sendNotification(payload, **kwargs)
    if self.main._settings.get_boolean(['auto_turn_off']):
      if not self.main.tcmd:
        self._logger.debug("Cannot auto-turn off")
        return
      self.main.tcmd.commandDict['/off']['cmd'](None,None,None, None)
